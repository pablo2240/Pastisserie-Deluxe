...
const Notificaciones: React.FC<NotificacionesProps> = ({ className }) => {
            ...
            if (user?.rol) {
                ...
                if (roles.includes('Repartidor')) {
                    processedData = processedData.sort((a: Notificacion, b: Notificacion) => {
                        ...
                        const aPrio = aTitulo.includes('asignad') ? 1 : 0;
                        const bPrio = bTitulo.includes('asignad') ? 1 : 0;
                        return bPrio - aPrio;
                    });
                }
            }
    ...
    const handleNotificacionClick = (notificacion: Notificacion) => {
        // Solo marcar como leída, sin redirección
                            ...
                            <ul className="divide-y divide-gray-50">
                                {notificaciones.map(notificacion => (
                                    <li
                                        ...
                                        className={`p-4 hover:bg-gray-50 transition-all cursor-default ${!notificacion.leida ? 'bg-blue-50/20' : ''}`}
                                    >
                                        <div className="flex gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[9px] text-gray-400 font-bold uppercase mt-2">
                                                    Hace {Math.max(0, Math.floor((new Date().getTime() - new Date(notificacion.fechaCreacion || new Date()).getTime()) / 60000))} min
                                                </p>
                                            </div>
                                        </div>
                                    </li>
