    ...
    <div className="animate-fade-in space-y-6">
                  ...
                  <Fragment key={pedido.id}>
                    <tr className={`hover:bg-gray-50 transition-colors ${selectedPedido?.id === pedido.id ? 'bg-[#7D2121]/5' : ''}`}>
                      ...
                      <td className="px-6 py-4">
                        ...
                        {formatMedellinDate(pedido.fechaPedido)}
                      </td>
      ...
      {selectedPedido && createPortal(
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-[95vw] md:w-full max-w-5xl flex flex-col max-h-[90vh] md:max-h-[85vh] overflow-hidden animate-scale-in">
            <div className="bg-[#7D2121] text-white px-6 md:px-8 py-5 flex justify-between items-center shrink-0">
              <div>
                <h2 className="text-xl font-bold flex items-center gap-2"><FiPackage /> Detalle del Pedido #{selectedPedido.id}</h2>
                <p className="text-white/70 text-xs mt-1">{formatMedellinDateTime(selectedPedido.fechaPedido)}</p>
              </div>
              <button
            ...
            <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8 overflow-y-auto flex-1 custom-scrollbar">
              {/* Info Cliente */}
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                    <FiPackage className="text-[#7D2121]" /> Repartidor Asignado
                  </h3>
                  {selectedPedido.estado === 'Entregado' ? (
                    <div className="w-full bg-gray-100 rounded-lg p-3 text-sm text-gray-500 border border-gray-200 cursor-not-allowed">
                      {repartidores.find(r => r.id === selectedPedido.repartidorId)?.nombre || 'Sin asignar'}
                      <span className="ml-2 text-[10px] text-gray-400 italic">(Pedido entregado - no editable)</span>
                    </div>
                  ) : (
                    <select
