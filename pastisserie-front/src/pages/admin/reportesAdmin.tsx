...
import {
  FiAlertCircle, FiTruck, FiX, FiUser, FiClock, FiEye
} from 'react-icons/fi';
import toast from 'react-hot-toast';
import { reclamacionesService, type Reclamacion } from '../../services/reclamacionesService';
import { enviosService, type Envio } from '../../services/enviosService';
import { formatMedellinDate, formatMedellinDateTime } from '../../utils/format';

const ReportesAdmin = () => {
  const [activeTab, setActiveTab] = useState<'reportes' | 'envios'>('reportes');
  ...
  const fetchEnvios = async () => {
    try {
      ...
      const response = await enviosService.getAllEnvios();
      setEnvios(response.data || []);
    } catch (error) {
      console.error(error);
      toast.error('Error al cargar solicitudes de envio');
    } finally {
  ...
  const updateEnvioEstado = async (id: number, estado: string) => {
    try {
      await enviosService.updateEstado(id, estado);
      toast.success('Estado del envio actualizado');
      fetchEnvios();
      setSelectedEnvio(null);
    } catch (error) {
      toast.error('Error al actualizar estado del envio');
    }
  };
  ...
  const getEnvioStatusStyle = (estado: string) => {
    switch (estado) {
      case 'Pendiente': return 'bg-yellow-100 text-yellow-700 border-yellow-200';
      case 'Asignado': return 'bg-blue-100 text-blue-700 border-blue-200';
      case 'EnCamino': return 'bg-indigo-100 text-indigo-700 border-indigo-200';
      ...
      case 'Devuelto': return 'bg-gray-100 text-gray-700 border-gray-200';
      default: return 'bg-gray-100 text-gray-700 border-gray-200';
    }
  };
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div>
        ...
        <p className="text-gray-500">Gestiona solicitudes de reporte y envio.</p>
      </div>
      ...
      <div className="flex border-b border-gray-200 gap-6">
        <button
          ...
          className={`pb-3 font-medium text-sm flex items-center gap-2 transition-colors border-b-2 ${activeTab === 'reportes' ? 'border-[#7D2121] text-[#7D2121]' : 'border-transparent text-gray-500'}`}
        >
          <FiAlertCircle /> Solicitud de Reporte ({reclamaciones.length})
        </button>
        <button
          onClick={() => setActiveTab('envios')}
          className={`pb-3 font-medium text-sm flex items-center gap-2 transition-colors border-b-2 ${activeTab === 'envios' ? 'border-[#7D2121] text-[#7D2121]' : 'border-transparent text-gray-500'}`}
        >
          <FiTruck /> Solicitud de Envio ({envios.length})
        </button>
      </div>
            ...
            <table className="w-full text-left text-sm text-gray-600">
              ...
              <tbody className="divide-y divide-gray-100">
                {loadingEnvios ? (
                  ...
                  <tr><td colSpan={7} className="text-center py-10 text-gray-400">No hay solicitudes de envio.</td></tr>
                ) : (
                  envios.sort((a, b) => b.id - a.id).map((e) => (
                    <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 font-bold text-[#7D2121]">#{e.id}</td>
                      <td className="px-6 py-4 flex items-center gap-2">
                        <div className="p-1 bg-gray-100 rounded text-gray-500"><FiUser /></div>
                        {e.nombreCliente || 'N/A'}
                      </td>
                      <td className="px-6 py-4 font-bold">#{e.pedidoId}</td>
                      <td className="px-6 py-4">{e.nombreRepartidor || 'Sin asignar'}</td>
                      <td className="px-6 py-4">{formatMedellinDate(e.fechaDespacho)}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-md text-xs font-bold border ${getEnvioStatusStyle(e.estado)}`}>
                          {e.estado}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button
                          onClick={() => setSelectedEnvio(e)}
                          className="bg-gray-100 hover:bg-[#7D2121] hover:text-white text-gray-600 px-3 py-1.5 rounded-lg flex items-center gap-2 transition-all font-bold text-xs mx-auto shadow-sm"
                        >
                          <FiEye /> Detalle
                        </button>
      ...
      {/* === MODAL DETALLE ENVIO === */}
      {selectedEnvio && createPortal(
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[85vh] overflow-hidden animate-scale-in">
            <div className="bg-[#7D2121] text-white px-6 py-5 flex justify-between items-center shrink-0">
              <div>
                <h2 className="text-xl font-bold flex items-center gap-2"><FiTruck /> Solicitud de Envio #{selectedEnvio.id}</h2>
                <p className="text-white/70 text-xs mt-1">Pedido #{selectedEnvio.pedidoId}</p>
              </div>
              <button onClick={() => setSelectedEnvio(null)} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                <FiX size={24} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5 overflow-y-auto flex-1 custom-scrollbar">
              <div className="bg-gray-50 rounded-xl p-5 border border-gray-100 space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-gray-800 text-sm">{selectedEnvio.nombreCliente || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 text-xs">Pedido:</span>
                  <span className="font-bold text-[#7D2121] text-sm">#{selectedEnvio.pedidoId}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 text-xs">Total Pedido:</span>
                  <span className="font-bold text-gray-800 text-sm">${selectedEnvio.totalPedido.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 text-xs">Repartidor:</span>
                  <span className="font-bold text-gray-800 text-sm">{selectedEnvio.nombreRepartidor || 'Sin asignar'}</span>
                </div>
                {selectedEnvio.numeroGuia && (
                  <div className="flex justify-between">
                    <span className="text-gray-500 text-xs">Numero de Guia:</span>
                    <span className="font-bold text-gray-800 text-sm">{selectedEnvio.numeroGuia}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-500 text-xs">Direccion:</span>
                  <span className="font-bold text-gray-800 text-sm">{selectedEnvio.direccionEnvio || 'Recogida en tienda'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 text-xs">Fecha Despacho:</span>
                  <span className="text-sm">{formatMedellinDateTime(selectedEnvio.fechaDespacho)}</span>
                </div>
                {selectedEnvio.fechaEntrega && (
                  <div className="flex justify-between">
                    <span className="text-gray-500 text-xs">Fecha Entrega:</span>
                    <span className="text-sm">{formatMedellinDateTime(selectedEnvio.fechaEntrega)}</span>
                  </div>
                )}
                <div className="flex justify-between items-center">
                  <span className="text-gray-500 text-xs">Estado:</span>
                  <span className={`px-2 py-1 rounded-md text-xs font-bold border ${getEnvioStatusStyle(selectedEnvio.estado)}`}>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {['Pendiente', 'Asignado', 'EnCamino', 'Entregado', 'Fallido', 'Devuelto'].map((estado) => (
                    <button
                      key={estado}
                      onClick={() => updateEnvioEstado(selectedEnvio.id, estado)}
                      disabled={selectedEnvio.estado === estado}
                      className={`px-3 py-2 rounded-lg text-xs font-bold transition-all border ${selectedEnvio.estado === estado
                        ? 'bg-[#7D2121] text-white border-[#7D2121] cursor-default'
                        : 'bg-white text-gray-600 border-gray-200 hover:border-[#7D2121] hover:text-[#7D2121]'
                        }`}
                    >
