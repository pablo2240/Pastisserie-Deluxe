...
interface CartContextType {
  ...
  cartTotal: number;
  isLoading: boolean;
  addToCart: (productoId: number | null, cantidad?: number, promocionId?: number) => Promise<void>;
  updateCartItem: (itemId: number, cantidad: number) => Promise<void>;
  removeFromCart: (itemId: number) => Promise<void>;
  clearCart: () => Promise<void>;
  compraMinima: number;
  maxUnidadesPorProducto: number;
  limitarUnidadesPorProducto: boolean;
}
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Store config fields
  const [compraMinima, setCompraMinima] = useState(15000);
  const [maxUnidadesPorProducto, setMaxUnidadesPorProducto] = useState(10);
  const [limitarUnidadesPorProducto, setLimitarUnidadesPorProducto] = useState(false);

  // Fetch business rules from store config
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const resp = await api.get('/configuracion');
        const config = resp.data?.data || {};
        setCompraMinima(config.compraMinima !== undefined ? Number(config.compraMinima) : 15000);
        setMaxUnidadesPorProducto(config.maxUnidadesPorProducto !== undefined ? Number(config.maxUnidadesPorProducto) : 10);
        setLimitarUnidadesPorProducto(!!config.limitarUnidadesPorProducto);
      } catch {
        setCompraMinima(15000);
        setMaxUnidadesPorProducto(10);
        setLimitarUnidadesPorProducto(false);
      }
    };
    fetchConfig();
  }, []);
  ...
  const addToCart = async (productoId: number | null, cantidad: number = 1, promocionId?: number) => {
    if (!isAuthenticated) {
      toast.error('Debes iniciar sesión para comprar');
      return;
    }

    // Validar que al menos se envíe productoId o promocionId
    if (!productoId && !promocionId) {
      toast.error('Error: debe especificar un producto o promoción');
      return;
    }
    ...
    // Restricción de unidades por producto (solo para productos, no para promociones independientes)
    if (limitarUnidadesPorProducto && productoId) {
      // Buscar el ítem actual de ese producto en el carrito
      const actualItem = carrito?.items?.find(item => item.productoId === productoId);
      const actualCantidad = actualItem ? actualItem.cantidad : 0;
      const nuevaCantidad = actualCantidad + cantidad;
      if (nuevaCantidad > maxUnidadesPorProducto) {
    ...
    setIsLoading(true);
    try {
      const request: AddToCartRequest = { productoId, cantidad, promocionId: promocionId || null };
