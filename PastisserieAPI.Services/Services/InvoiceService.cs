            ...
            container.PaddingVertical(1, Unit.Centimetre).Column(column =>
            {
                ...
                column.Item().Row(row =>
                {
                    ...
                    row.RelativeItem().AlignRight().Column(col =>
                    {
                        ...
                        // Conversión de fecha UTC a hora de Colombia (UTC-5)
                        DateTime fechaColombia;
                        try {
                            var bogotaZone = TimeZoneInfo.FindSystemTimeZoneById("SA Pacific Standard Time");
                            fechaColombia = TimeZoneInfo.ConvertTimeFromUtc(
                                DateTime.SpecifyKind(pedido.FechaPedido, DateTimeKind.Utc), bogotaZone);
                        } catch {
                            try {
                                ...
                                fechaColombia = pedido.FechaPedido.AddHours(-5);
                            }
                        }
