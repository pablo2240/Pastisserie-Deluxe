...
const CartSidebar = ({ isOpen, onClose }: Props) => {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-black/30 backdrop-blur-sm z-[150] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      />

      <aside className={`fixed right-0 top-0 h-full w-full max-w-md bg-white z-[160] shadow-2xl transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div key={item.id} className="flex gap-4 bg-gray-50 p-4 rounded-xl items-center shadow-sm border border-gray-100">
                  ...
                  <div className="flex-grow">
                    ...
                    {item.promocionId && item.nombrePromocion && (
                      <span className="inline-block px-2 py-0.5 bg-red-100 text-red-700 text-[10px] font-bold rounded-full uppercase tracking-wider mt-0.5">
                        {item.nombrePromocion}
                      </span>
                    )}
                    <p className="text-gray-500 text-xs">Cant: {item.cantidad}{item.promocionId ? ' (Max 3)' : ''}</p>
                    <div className="flex items-center gap-2">
                      {item.precioOriginal && item.precioOriginal !== item.precioUnitario && (
                        <span className="text-gray-400 line-through text-xs">{formatCurrency(item.precioOriginal)}</span>
                      )}
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50">
              <div className="space-y-2 mb-4">
                <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                  ...
                  <span className="text-xl font-bold text-[#7D2121]">
