...
El sistema PastisserieDeluxe tiene **3 roles principales** definidos en la base de datos y utilizados en la autenticación:
...
| `Pedidos` | SELECT (propios), INSERT, DELETE (propios si aplica) |
| `PedidoItems` | SELECT (propios) |
...
| `MetodosPagoUsuario` | SELECT, INSERT, UPDATE, DELETE (propios) |
| `Reviews` | INSERT (propias), SELECT (todas) |
...
| `Pedidos` | SELECT (asignados), UPDATE (asignados) |
| `PedidoItems` | SELECT (de pedidos asignados) |
| `Envios` | SELECT (asignados), UPDATE (asignados) |
| `DireccionEnvio` | SELECT (de pedidos asignados) |
