...
const ProductCard: React.FC<ProductCardProps> = ({ product, tiendaStatus, statusLoading }) => {
  ...
  const isMaintenance = !!(status && status.usarControlHorario === false && status.sistemaActivoManual === false);
  const isConnectionError = status === null && !statusLoading;
    ...
    <>
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          <div className="mt-auto pt-3">
            ...
            <button
              onClick={(e) => {
                e.preventDefault();
                if (isClosed || isMaintenance || isConnectionError) {
              ...
              className={`w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all duration-300 btn-premium uppercase tracking-widest 
                ${(isClosed || isMaintenance || isConnectionError)
                  ...
                  : 'bg-patisserie-dark text-white hover:bg-patisserie-red disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed'}`}
            >
              {statusLoading ? (
                ...
                <><FiClock className="text-lg" /> Ver Horarios</>
              ) : isConnectionError ? (
                <><FiX className="text-lg" /> Error de Conexión</>
              ) : (
      ...
      {showHoursModal && createPortal(
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-[#5D1919]/20 backdrop-blur-md animate-fade-in p-4" onClick={() => setShowHoursModal(false)} role="dialog">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-[95vw] md:w-full max-w-md md:max-w-lg overflow-hidden flex flex-col max-h-[90vh] md:max-h-[85vh] animate-scale-in border border-gray-100" onClick={e => e.stopPropagation()}>
            <div className="bg-[#7D2121] text-white p-6 md:p-8 relative shrink-0">
              <button
              ...
              {!status?.usarControlHorario && !status?.sistemaActivoManual ? (
                ...
                <div className="space-y-2">
                  {status.horariosPorDia.map(h => {
                    // Obtención de fecha en Medellín para resaltar "Hoy" correctamente
                    const medellinDate = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Bogota" }));
                    const isHoy = medellinDate.getDay() === h.diaSemana;
                    return (
            ...
            <div className="px-6 py-6 md:px-8 md:py-8 bg-white border-t border-gray-50 flex shrink-0">
              <button
                onClick={() => setShowHoursModal(false)}
                className="w-full py-4 bg-gray-900 text-white font-black uppercase tracking-[0.2em] text-[10px] rounded-2xl hover:bg-[#7D2121] transition-all"
              >
                Comprendido
              </button>
            </div>
