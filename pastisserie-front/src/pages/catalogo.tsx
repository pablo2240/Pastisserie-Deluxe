...
const Catalogo = () => {
    ...
    const productosFiltrados = (productos || [])
        .filter(p => {
            ...
            const coincidePrecio = (p.precio ?? 0) >= (precioRango?.[0] ?? 0) && (p.precio ?? 0) <= (precioRango?.[1] ?? 10000000);
            return coincideCategoria && coincideBusqueda && coincidePrecio;
        })
        .sort((a, b) => {
            if (!a || !b) return 0;
            if (ordenarPor === 'precio-asc') return (a.precio ?? 0) - (b.precio ?? 0);
            if (ordenarPor === 'precio-desc') return (b.precio ?? 0) - (a.precio ?? 0);
