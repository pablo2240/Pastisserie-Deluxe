...
namespace PastisserieAPI.API.Controllers
{
    ...
    public class PedidosController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreatePedidoRequestDto request)
        {
            ...
            _logger.LogInformation("Create Pedido: Token userId claim = {UserIdClaim}", userIdStr);

            if (string.IsNullOrEmpty(userIdStr) || !int.TryParse(userIdStr, out int userId))
            {
                _logger.LogWarning("Create Pedido: Usuario no identificado en token. Claim value: {ClaimValue}", userIdStr);
                return Unauthorized(ApiResponse<string>.ErrorResponse("Usuario no identificado en token"));
            }

            _logger.LogInformation("Create Pedido: Creando pedido para usuarioId={UserId}", userId);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                var result = await _pedidoService.DeleteAsync(id, userId, isAdmin);
                if (!result) return NotFound(ApiResponse<string>.ErrorResponse("Pedido no encontrado"));
                return Ok(ApiResponse<string>.SuccessResponse("Pedido eliminado exitosamente"));
            }
            catch (Exception ex)
            {
